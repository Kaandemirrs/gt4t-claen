...
// Partner card Read More açılır/kapanır fonksiyonu
function setupReadMoreButtons() {
  document.querySelectorAll('.read-more-btn').forEach(function(btn) {
    // Aynı butona birden fazla event eklenmesini önle
...
function setupHamburgerMenu() {
  ...
  // Overlay'i bir kez oluştur, tekrar tekrar ekleme
...
const reinitializeDynamicScripts = () => {
  ...
  setupHamburgerMenu(); // <-- Bunu ekledim
...
document.addEventListener('DOMContentLoaded', function() {
  function addGreenFlashOnClick(selector, mailto) {
    document.querySelectorAll(selector).forEach(function(card) {
      card.addEventListener('click', function() {
        card.classList.remove('card-green-flash');
        setTimeout(function() {
          card.classList.add('card-green-flash');
        }, 10);
  ...
  addGreenFlashOnClick('.coordinator-card', 'ali.2.tavakoli@samk.fi');
  addGreenFlashOnClick('.timeline-card');
});
